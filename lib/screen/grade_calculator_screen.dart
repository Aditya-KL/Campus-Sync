// lib/screen/grade_calculator_screen.dart
//
// Uses FirebaseService for all Firestore access.
//
// ── PAST SEMESTERS DB STRUCTURE ──────────────────────────────
// students/{uid}/pastSemesters/"Semester 1"
// students/{uid}/pastSemesters/"Semester 2"  … etc.
//
// Each doc: { semester, spi, "spi ", cpi, credit, lockedAt }
//
// ── SEMESTER ROLLOVER ────────────────────────────────────────
// Odd  sems end Dec 31 → new semester starts Jan  1
// Even sems end May 31 → new semester starts Jun  1
//
// On _fetchAcademicData():
//   1. FirebaseService.checkAndRollSemester() checks if the boundary
//      has passed and the current sem has not been archived yet.
//   2. If yes: archives current sem → increments currentSemester
//      → returns new semester number.
//   3. Grade calculator then fetches curriculum for the NEW semester.
...
class _GradeCalculatorScreenState extends State<GradeCalculatorScreen> {
  ...
  // One TextEditingController per past semester row (SGPA editable)
  ...
  /// Recompute the running CPI column from current SPI values.
  ...
  /// CGPA across ALL semesters (past + current).
  ...
  Future<void> _fetchAcademicData() async {
    try {
      ...
      // ── 1. Student doc ────────────────────────────────────────
      final sdSnap = await _svc.getStudentDoc(uid);
      final sd = sdSnap.data();
      if (sd == null) {
        setState(() => _isLoading = false);
        return;
      }
      ...
      // ── 4. Past semesters from SUBCOLLECTION ──────────────────
      // Path: students/{uid}/pastSemesters/"Semester N"
      final rawPastList = await _svc.getPastSemesters(uid);

      // Build lookup: semNumber → raw map
      final Map<int, Map<String, dynamic>> dbSemMap = {};
      for (final m in rawPastList) {
        final sem = (m['semester'] as int?) ?? 0;
        if (sem > 0) dbSemMap[sem] = m;
      }

      // Build PastSemester list for sems 1 … (currentSem - 1)
      final List<PastSemester> pastSems = [];
      for (int sem = 1; sem < currentSem; sem++) {
        final credit = FirebaseService.creditForSem(creditsDoc, sem);
        ...
        pastSems.add(
          PastSemester(
            ...
            credit: credit,
            spi: dbRow != null
                ? ((dbRow['spi'] ?? dbRow['spi '] ?? 8.0) as num).toDouble()
                : 8.0, // placeholder until student fills it in
            cpi: dbRow != null
                ? ((dbRow['cpi'] ?? 0.0) as num).toDouble()
                : 0.0,
          ),
        );
      }
      ...
      // ── 5. Current semester courses ───────────────────────────
      // Load courses FIRST so we have a real SPI to archive if rollover fires.
      final currSnap = await _svc.getCurriculumDoc(
        branch: branch,
        semester: currentSem,
      );
      ...
      if (currSnap != null && currSnap.exists) {
      ...
      // ── 6. Check semester rollover ────────────────────────────
      // Now that courses are loaded we have a real SPI to archive.
      // Compute SPI from loaded courses.
      double tentativeSpi = 0.0;
      {
        double sumCP = 0, sumC = 0;
        ...
        if (sumC > 0) tentativeSpi = sumCP / sumC;
      }

      // Compute tentative CGPA (past + current)
      double tentativeCgpa = 0.0;
      {
        double sumCP = 0, sumC = 0;
        ...
        if (sumC > 0) tentativeCgpa = sumCP / sumC;
      }

      final int currentCredit = FirebaseService.creditForSem(
        creditsDoc,
        currentSem,
      );

      final newSem = await _svc.checkAndRollSemester(
        uid,
        currentSem: currentSem,
        spiForCurrentSem: tentativeSpi,
        cpiForCurrentSem: tentativeCgpa,
        creditForCurrentSem: currentCredit,
      );

      // If rollover happened — semester bumped, reload courses for new semester
      List<CourseGrade> finalCourses = courses;
      if (newSem != currentSem) {
        currentSem = newSem;

        // Refetch past sems (the just-archived one is now in DB)
        final refreshed = await _svc.getPastSemesters(uid);
        pastSems.clear();
        for (final m in refreshed) {
          final sem = (m['semester'] as int?) ?? 0;
          final credit = FirebaseService.creditForSem(creditsDoc, sem);
          if (sem > 0 && sem < currentSem) {
            pastSems.add(
              PastSemester(
                semester: sem,
                credit: credit,
                spi: ((m['spi'] ?? m['spi '] ?? 8.0) as num).toDouble(),
        ...
        // Load courses for the NEW semester
        final newCurrSnap = await _svc.getCurriculumDoc(
          branch: branch,
          semester: currentSem,
        );
        finalCourses = [];
        if (newCurrSnap != null && newCurrSnap.exists) {
          for (final raw in (newCurrSnap.data()?['courses'] ?? []) as List) {
            final code = (raw['code'] ?? '') as String;
            final name = (raw['name'] ?? '') as String;
            final credit = ((raw['credit'] ?? 0) as num).toDouble();
            // New semester — no saved grades yet
            finalCourses.add(
              CourseGrade(
                id: code,
                name: name,
                credit: credit,
                marksObtained: 100.0,
                expectedGrade: 'AA', // default placeholder
              ),
            );
          }
        }
      ...
      // ── 7. Set state ──────────────────────────────────────────
      setState(() {
        _currentSemester = currentSem;
        _pastSemesters = pastSems;
        _currentCourses = finalCourses;
        _isLoading = false;
      });

      _recomputePastCGPA();

      // ── 8. Build SGPA text controllers (one per past row) ─────
      _spiCtrls.forEach((_, c) => c.dispose());
      _spiCtrls.clear();

      for (final s in pastSems) {
        final ctrl = TextEditingController(text: s.spi.toStringAsFixed(2));
        ...
        ctrl.addListener(() {
          ...
          _recomputePastCGPA();
          if (mounted) setState(() {});
          // Persist to subcollection doc "Semester N"
          _svc
              .updatePastSemesterSpi(
                uid,
                semester: s.semester,
                spi: clamped,
                cpi: s.cpi,
              )
              .catchError((e) => debugPrint('SPI save error: $e'));
        });

        _spiCtrls[s.semester] = ctrl;
      }
    } catch (e, st) {
    ...
    showDialog(
      ...
      builder: (_) => _WeightageDialog(
        ...
        onSave: (predicted, grade, data) async {
          setState(() {
            course.marksObtained = predicted;
            course.expectedGrade = grade;
            course.advancedGrading = data;
          });
          final user = FirebaseAuth.instance.currentUser;
          if (user != null) {
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: _StatCard(
              label: 'SGPA',
              sub: 'Sem $_currentSemester',
              value: spi > 0 ? spi.toStringAsFixed(2) : '--',
              dark: true,
            ),
          ),
          const SizedBox(width: 12),
  ...
  Widget _currentSemHeader() => Padding(
    padding: const EdgeInsets.fromLTRB(20, 28, 20, 12),
    child: Row(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        // Semester number badge — prominent pill
        Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (course.advancedGrading != null)
                    Text(
                      '${course.marksObtained.toStringAsFixed(1)}%',
                      style: const TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w600,
                        color: _muted,
              ...
              return Column(
                children: [
                  Padding(
                    ...
                    child: Row(
                      children: [
                        // Semester badge — label shows "Semester N"
                        Expanded(
                          child: Center(
                            child: Container(
                              padding: const EdgeInsets.symmetric(
                        ...
                        // CPI — auto-derived, read-only
                        Expanded(
                          child: Text(
                            s.cpi.toStringAsFixed(2),
                            textAlign: TextAlign.center,
                            style: const TextStyle(
                              fontSize: 15,
                              fontWeight: FontWeight.w900,
...
class _StatCard extends StatelessWidget {
  ...
  static const _muted = Color(0xFF8A8F9D);
...
// ─────────────────────────────────────────────────────────────
// WEIGHTAGE DIALOG  (unchanged from original)
// ─────────────────────────────────────────────────────────────
...
class _WeightageDialogState extends State<_WeightageDialog> {
  ...
  void _init() {
    ...
    _tWCtrl = TextEditingController(
      text: (d['theory_weight'] as double).toStringAsFixed(0),
    );
    _lWCtrl = TextEditingController(
      text: (d['lab_weight'] as double).toStringAsFixed(0),
    );
    _theory = (d['theory'] as List)
  ...
  void _initCtrls(List<Map<String, dynamic>> items, String pfx) {
    for (int i = 0; i < items.length; i++) {
      final k = '${pfx}_$i';
      _wC[k] = TextEditingController(
        text: (items[i]['weight'] as double).toStringAsFixed(0),
      );
      _tC[k] = TextEditingController(
        text: (items[i]['total'] as double).toStringAsFixed(0),
      );
      _wC[k]!.addListener(() {
        items[i]['weight'] = double.tryParse(_wC[k]!.text) ?? 0.0;
        setState(() {});
      });
      _tC[k]!.addListener(() {
        items[i]['total'] = double.tryParse(_tC[k]!.text) ?? 0.0;
        if ((items[i]['obtained'] as double) > (items[i]['total'] as double)) {
          items[i]['obtained'] = items[i]['total'];
        }
        setState(() {});
      });
    }
  ...
  double get _predicted {
    ...
    for (final c in _theory) {
      final w = c['weight'] as double;
      final t = c['total'] as double;
      final o = c['obtained'] as double;
      if (t > 0) tScore += (o / t) * w;
    }
    for (final c in _lab) {
      final w = c['weight'] as double;
  ...
  Widget _errorBanner() {
    ...
    if (!_overallOk)
      msgs.add(
        'Theory + Lab = ${(_tW + _lW).toStringAsFixed(0)}% (must be 100%)',
      );
    ...
    return List.generate(items.length, (i) {
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white,
          border: Border.all(
            color: w > 0 ? _yellow.withOpacity(0.35) : _bg,
            width: 1.5,
          ),
          borderRadius: BorderRadius.circular(12),
        ),
        child: Column(
          ...
          children: [
            Row(
              children: [
                ...
                _miniField(
                  'Wt',
                  _wC[k]!,
                  w > 0 ? _yellow.withOpacity(0.2) : _bg,
                ),
                const SizedBox(width: 8),
            ...
            if (enabled) ...[
              ...
              const SizedBox(height: 4),
              Text(
                w == 0 ? 'Set weight > 0 to enable' : 'Set total > 0 to enable',
                style: const TextStyle(
                  fontSize: 10,
                  color: _muted,
  ...
  Widget _miniField(String label, TextEditingController ctrl, Color fill) =>
      Column(
        children: [
          Text(
            label,
          ...
          child: ElevatedButton(
            ...
            child: Text(
              _canSave
                  ...
                  : 'Set Weightage accordingly to Save',
